...
interface Event {
  ...
  department: string;
  status: string; // Changed to generic string to handle dynamic backend changes
  attendees: number;
}
...
// Fixed mapping to handle various status formats (case-insensitive)
...
// Safety fallback for unknown statuses to prevent crash
const getStatusStyles = (status: string) => {
  return statusColors[status] || { bg: "#F9FAFB", text: "#6B7280", border: "#E5E7EB" };
};
  ...
  return (
    <div className="page-enter space-y-6">
      ...
      <div className="space-y-4">
        {eventList.map((event) => {
          const styles = getStatusStyles(event.status); // Call safety helper
          return (
            <div key={event.id} className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 hover:border-blue-300 transition-colors text-left">
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className="text-lg font-semibold text-gray-800">{event.title}</h3>
                    <Badge variant="outline" style={{ backgroundColor: styles.bg, color: styles.text, borderColor: styles.border }}>
                      {event.status}
                    </Badge>
                  </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {(event.status === "Pending Approval" || event.status === "pending") && (
                    <div className="flex gap-2">
                  ...
                  {(event.status === "Approved" || event.status === "approved" || event.status === "Rejected" || event.status === "rejected") && (
      ...
      <Dialog open={!!selectedEvent} onOpenChange={() => setSelectedEvent(null)}>
        <DialogContent className="bg-white max-w-2xl text-left">
          <DialogHeader><DialogTitle>{selectedEvent?.title}</DialogTitle></DialogHeader>
          {selectedEvent && (
            <div className="space-y-4 pt-4 text-sm">
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 bg-gray-50 rounded-lg text-left"><p className="text-xs text-gray-500">Event ID</p><p className="font-mono text-blue-600">EVT-{selectedEvent.id}</p></div>
                <div className="p-3 bg-gray-50 rounded-lg text-left"><p className="text-xs text-gray-500">Venue</p><p>{selectedEvent.venue}</p></div>
              </div>
              <div className="p-4 bg-blue-50 rounded-lg">
                <p className="text-xs text-blue-600 mb-2 font-medium text-left">Coordinator Details</p>
      ...
      <Dialog open={!!historyEvent} onOpenChange={() => setHistoryEvent(null)}>
        <DialogContent className="bg-white text-left">
          <DialogHeader><DialogTitle>Event History</DialogTitle></DialogHeader>
          {loadingHistory ? <Loader2 className="animate-spin mx-auto my-4 text-blue-600" /> : (
            <div className="space-y-3 pt-4">
              {eventHistory.length > 0 ? eventHistory.map((item, i) => (
                <div key={i} className="p-3 bg-gray-50 rounded-lg text-left">
              ...
              )) : <p className="text-sm text-gray-500 text-center">No history records found.</p>}
            </div>
          )}
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!changeDecisionEvent} onOpenChange={() => setChangeDecisionEvent(null)}>
        <DialogContent className="bg-white">
          <DialogHeader className="text-left"><DialogTitle>Change Decision</DialogTitle></DialogHeader>
          <div className="space-y-4 pt-2">
            <Textarea value={rejectionReason} onChange={(e) => setRejectionReason(e.target.value)} placeholder="Reason for change..." className="text-left" />
            <div className="flex flex-col gap-2">
              ...
              {changeDecisionEvent?.status.toLowerCase().includes("reject") ? (
                <Button className="bg-green-600 hover:bg-green-700" onClick={() => handleStatusUpdate(changeDecisionEvent!.id, "Approved", rejectionReason)} disabled={!rejectionReason.trim()}>Change to Approved</Button>
              ) : (
                <Button className="bg-red-600 hover:bg-red-700" onClick={() => handleStatusUpdate(changeDecisionEvent!.id, "Rejected", rejectionReason)} disabled={!rejectionReason.trim()}>Change to Rejected</Button>
              )}
            </div>
          </div>
        </DialogContent>
